...
# === Kontrol kamera (auto exposure, sharpness) ===
picam2.set_controls({
    "AeEnable": True,        # Auto Exposure aktif
    "AwbEnable": True,       # Auto White Balance aktif
    "ExposureValue": 1.9,      # EV compensation (coba 0.0 s/d 4.0)
    "Sharpness": 1.5         # Default = 1.0 (coba 1.2–2.0)
})
...
def apply_zoom(factor):
    ...
    global zoom_factor
    zoom_factor = max(1.0, min(factor, 5.0))  # batasi 1x–5x zoom

    center_x, center_y = sensor_w // 2, sensor_h // 2
    new_width = int(sensor_w / zoom_factor)
    new_height = int(new_width / preview_aspect)

    # Safety check supaya ROI tidak keluar sensor
    if new_height > sensor_h:
        new_height = sensor_h
        new_width = int(new_height * preview_aspect)
    if new_width > sensor_w:
        new_width = sensor_w

    roi_x = max(0, center_x - new_width // 2)
    roi_y = max(0, center_y - new_height // 2)

    if roi_x + new_width > sensor_w:
        roi_x = sensor_w - new_width
    if roi_y + new_height > sensor_h:
        roi_y = sensor_h - new_height

    roi = (roi_x, roi_y, new_width, new_height)
    picam2.set_controls({"ScalerCrop": roi})
    print(f"Zoom factor: {zoom_factor:.1f}x | ROI: {roi}")
...
def enhance_image(img):
    # Tingkatkan kontras & pencahayaan dengan CLAHE
