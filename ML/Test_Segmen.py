...
def adjust_brightness(img, value=-40):
...
def checkParkingSpaceYOLO(img, posList, model, update_firebase=False):
    ...
    for result in results:
        # === Tambahkan overlay hijau dari segmentation mask
    ...
    # === Klasifikasi ukuran mobil dengan Fuzzy C-Means
    if len(car_areas) >= 3:
        ...
        label_dict = {
            cluster_urutan[0]: "Kecil",
            cluster_urutan[1]: "Normal",
            cluster_urutan[2]: "Besar"
        }

        for i in range(len(car_boxes)):
            ukuran = label_dict[cluster_membership[i]]
            cx, cy = centroids[i]
            cv2.putText(img, ukuran, (cx - 30, cy + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
    else:
        for (cx, cy) in centroids:
            cv2.putText(img, "Mobil", (cx - 30, cy + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 2)

    # === Deteksi slot parkir kosong/terisi
